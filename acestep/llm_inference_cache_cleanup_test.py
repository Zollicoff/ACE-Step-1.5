...
class LlmAcceleratorCacheCleanupTests(unittest.TestCase):
    ...
    def test_syncs_then_clears_cuda(self):
        ...
        with patch("torch.cuda.is_available", return_value=True), \
             ...
             patch("torch.cuda.empty_cache", side_effect=lambda: call_order.append("clear")):            handler._clear_accelerator_cache()
    ...
    def test_noop_when_device_is_cpu(self):
        """Method should be a safe no-op when device is CPU and no GPU available."""
        handler = LLMHandler()
        handler.device = "cpu"
