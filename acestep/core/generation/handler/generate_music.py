...
class GenerateMusicMixin:
    ...
    def generate_music(
        ...
        try:
            ...
            # For source-audio tasks the output duration must match the source
            # audio.  Override any caller-supplied audio_duration so every
            # downstream consumer (VRAM check, metadata, padding, progress)
            ...
            if processed_src_audio is not None and task_type in (
                ...
                if audio_duration is not None and audio_duration != src_duration:
                    logger.info(
                        ...
                        "audio_duration={:.1f} with src_audio duration={:.1f}",
                        task_type, float(audio_duration), src_duration,
                    )
